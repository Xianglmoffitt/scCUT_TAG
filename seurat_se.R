...
# input macs2 peaks
macs_peak <- read.table("PBMC/K27ac_PBMC_l1/rose_out/peak_l1_Gateway_SuperEnhancers.bed",sep="\t",header=F)
...
pbmc <- FindNeighbors(object = pbmc, reduction = 'lsi', dims = 1:10)
...
p1 <- DimPlot(object = pbmc, label = TRUE) + NoLegend() + ggtitle("scCUT&TAG-seq (SE)")
...
plot2 <- DimPlot(
  ...
  label = TRUE,
  repel = TRUE) + NoLegend() + ggtitle('scCUT&TAG-sseq(Macs2)')
